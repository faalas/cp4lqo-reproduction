...
class DQN:
    ...
    def select_action(self, env, need_random=True):
        ...
        ###print("[DQN/select_action]: eps_threshold: ", eps_threshold)
        ###print('[DQN/select_action]: select_action: ',sample > eps_threshold)
        # action_list
        if sample > eps_threshold:
            # action_list, chosen_action,all_action
    ...
    # Inherit from the select_action()
    # Return only one more argument:
    def hanwen_select_action(self, env, need_random=True):
        ...
        action_list = env.allAction(self.policy_net)
        action_batch = torch.cat([x[0] for x in action_list], dim=1) # action_cost from action list
        print("\t[DQN/select_action]: action_list: ", action_list)
        # print('\t[DQN/select_action]: action_batch: ', action_batch)
        ...
        if sample > eps_threshold:
            # action_list, chosen_action,all_action
            target = torch.argmin(action_batch, dim=1)
            return action_batch, action_list[target[0]][0], action_list[target[0]][1], [x[1] for x in action_list]
        else:
            target_idx = random.randint(0, len(action_list) - 1)
            return action_batch, action_list[target_idx][0], action_list[target_idx][1], [x[1] for x in action_list]
